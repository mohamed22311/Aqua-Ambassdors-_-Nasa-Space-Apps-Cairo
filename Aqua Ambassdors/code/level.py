...
class Level:
	def __init__(self,current_level,surface,create_overworld,change_coins,change_health):
		...
		# decoration 
		self.sky = Sky(8,'level',self.current_level)
		...
		for row_index, row in enumerate(layout):
			for col_index,val in enumerate(row):
				if val != '-1':
					...
					if type == 'terrain':
						img = 'graphics/terrain/'+chr(self.current_level+48)+'/terrain_tiles.png'
					...
					if type == 'crates':
						sprite = Crate(tile_size,x,y,self.current_level)
	...
	def player_setup(self,layout,change_health):
		for row_index, row in enumerate(layout):
			for col_index,val in enumerate(row):
				...
				y = row_index * tile_size
				if val == '0':
					sprite = Player((x,y),self.display_surface,self.create_jump_particles,change_health,self.current_level)
					self.player.add(sprite)
				if val == '1':
					hat_surface = pygame.image.load('graphics/characters/0/hat.png').convert_alpha()
	...
	def check_coin_collisions(self):
		...
		if collided_coins:
			...
			for coin in collided_coins:
				if coin.value == 5:
					img = 'graphics/questions/' + chr(self.current_level+48) + '/' + chr(self.question+48) + '.png'
					msgbox('','', image=img)
					self.question += 1
					if(self.question > 2):
						self.question = 0
		...
		if enemy_collisions:
			for enemy in enemy_collisions:
				...
				if enemy_top < player_bottom < enemy_center and self.player.sprite.direction.y >= 0:
					...
					explosion_sprite = ParticleEffect(enemy.rect.center,'explosion',self.current_level)
					self.explosion_sprites.add(explosion_sprite)
					enemy.kill()
				else:
					###############################
					#vid = Video('graphics/vid/a.mp4')
